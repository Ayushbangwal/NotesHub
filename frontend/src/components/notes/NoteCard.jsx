...
import {
  ...
  User, FileText, Share2, Check, Link2, Flag, TrendingUp
} from 'lucide-react'
...
const NoteCard = ({ note, animate = true }) => {
  ...
  const renderStars = (rating) => {
    return [1, 2, 3, 4, 5].map(i => (
      <Star
        key={i}
        className={`h-3 w-3 ${i <= Math.round(rating) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-600'}`}
      />
    ))
  }
    ...
    <>
      ...
      >
        <Link to={`/notes/${note._id}`} className="block h-full">
          <div className="relative h-full flex flex-col rounded-2xl overflow-hidden border border-white/[0.07] bg-gradient-to-b from-[#1a1d2e] to-[#13151f] shadow-xl transition-shadow duration-300 group-hover:shadow-primary-500/10 group-hover:shadow-2xl">

            {/* Top glow bar */}
            <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-primary-500 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500" />

            {/* Subtle background glow on hover */}
            <div className="absolute inset-0 bg-primary-500/[0.03] opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none rounded-2xl" />

            {/* === HEADER SECTION === */}
            <div className="p-5 pb-3 flex-1 relative">

              {/* Top row: file icon + subject badge + actions */}
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-2.5">
                  {/* File type icon with bg */}
                  <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/[0.08] flex items-center justify-center text-xl flex-shrink-0">
                    {getFileIcon(note.fileType)}
                  </div>
                  <div className="flex flex-col gap-1">
                    <span className={`px-2.5 py-0.5 text-[10px] font-semibold text-white rounded-full tracking-wide ${getSubjectColor(note.subject)} shadow-sm`}>
                      {note.subject}
                    </span>
                ...
                <div className="flex items-center gap-0.5">
                  ...
                  >
                    <Bookmark className={`h-4 w-4 transition-all duration-200 ${isBookmarked ? 'fill-current' : ''}`} />
                  </motion.button>
                </div>
              </div>

              {/* Title */}
              <h3 className="text-[15px] font-semibold text-gray-100 mb-2 line-clamp-2 leading-snug group-hover:text-primary-300 transition-colors duration-200">
              ...
              {note.tags?.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {note.tags.slice(0, 3).map((tag, i) => (
                    <span
                      key={i}
                      className="px-2 py-0.5 text-[11px] bg-white/[0.05] text-gray-400 rounded-md border border-white/[0.06] hover:bg-primary-500/15 hover:text-primary-300 hover:border-primary-500/30 transition-all duration-200 cursor-default"
                    >
                      #{tag}
                    </span>
                  ))}
                  {note.tags.length > 3 && (
                    <span className="px-2 py-0.5 text-[11px] bg-white/[0.03] text-gray-600 rounded-md border border-white/[0.05]">
            ...
            <div className="px-5 py-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[12px] font-semibold text-yellow-400">{note.averageRating.toFixed(1)}</span>
              </div>

              {/* Downloads */}
              <div className="flex items-center gap-1 text-gray-500">
                <TrendingUp className="h-3 w-3 text-emerald-500" />
                <span className="text-[12px] text-gray-400 font-medium">{note.downloads}</span>
                <span className="text-[11px] text-gray-600">downloads</span>
              </div>
            </div>
            ...
            <div className="px-5 py-2.5 flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[12px] text-gray-400">{note.uploadedBy?.username || 'Anonymous'}</span>
              </div>
              <div className="flex items-center gap-1 text-gray-600">
                <Calendar className="h-3 w-3" />
                <span className="text-[11px]">{formatDate(note.createdAt)}</span>
              </div>
            </div>
            ...
            <div className="px-5 py-4 flex items-center gap-2">
              ...
              <motion.button
                ...
                className="flex-1 flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl bg-primary-600 hover:bg-primary-500 text-white text-[13px] font-medium transition-all duration-200 disabled:opacity-50"
              >
              ...
              <motion.button
                ...
                title="Preview"
                className="p-2 rounded-xl border border-white/[0.08] bg-white/[0.03] text-gray-400 hover:text-primary-400 hover:border-primary-500/30 hover:bg-primary-500/10 transition-all duration-200"
              >
                ...
                <motion.button
                  ...
                  className={`p-2 rounded-xl border transition-all duration-200 ${
                    copied
                      ...
                      : 'border-white/[0.08] bg-white/[0.03] text-gray-400 hover:text-primary-400 hover:border-primary-500/30 hover:bg-primary-500/10'
                  }`}
                >
                      ...
                      {[
                        ...
                        <React.Fragment key={i}>
                          ...
                          <button
                            ...
                            className="flex items-center gap-3 w-full px-4 py-2.5 text-sm transition-all duration-150 hover:bg-white/5"
                          >
                            <span className={`flex items-center justify-center w-7 h-7 rounded-lg ${item.iconBg}`}>
                              {item.icon}
                            </span>
                            <span className={`text-[13px] ${item.labelColor}`}>{item.label}</span>
                          </button>
                        </React.Fragment>
                      ))}
                    </motion.div>
